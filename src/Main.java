    ...
    while (!done) {
      ...
      case 3:
        ...
        if (found != null) {
          ...
          if (available == qty) {
            List<Stock> removedStock =
                Stock.removeStock(found.getId(), available);
            for (Stock s : removedStock) {
              System.out.println("Removido: " + s.getDate() + " - " +
                                 s.getQty());
            }
          } else if (available == 0) {
            ...
            while (error) {
              System.out.print(
                  "\nEstoque insuficiente. Deseja retirar o estoque disponivel (" +
                  available + " em estoque)? (S/N) ");
              input = new Scanner(System.in);
              String select = input.nextLine();
              if (select.equals("S")) {
                // System.out.println(Stock.removeStock(found.getId(),
                // available));
                List<Stock> removedStock =
                    Stock.removeStock(found.getId(), available);
                for (Stock s : removedStock) {
